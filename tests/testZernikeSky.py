...
import lsst.sims.skybrightness_pre.zernike as zernike
from lsst.sims.skybrightness_pre.zernike.zernike import TELESCOPE
...
class TestZenikeFitDrivers(unittest.TestCase):
    ...
    def setUp(self):
        self.cut_pre_data_dir = os.path.join(
...
#    @unittest.skip("skipping test_fit_pre")
    def test_fit_pre(self):
        npy_fname = os.path.join(
            self.cut_pre_data_dir,
            self.test_data_base_fname + '.npy')
        npz_fname = os.path.join(
            self.cut_pre_data_dir,
            self.test_data_base_fname + '.npz')
...
class TestZernikeSky(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fname = os.path.join(
            self.cut_pre_data_dir, 'zernsky.h5')
